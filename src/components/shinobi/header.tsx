...
export function ShinobiHeader() {
  ...
  return (
    <header className="absolute inset-x-0 top-0 z-40 px-5 py-5 text-white md:px-10">
      <div className="mx-auto grid max-w-[1500px] grid-cols-[auto_1fr_auto] items-center gap-4">
        ...
        >
          <Image
            ...
            width={208}
            height={56}
            className="h-12 w-40 object-contain object-left md:h-14 md:w-52"
          />
        </Link>

        <div className="hidden justify-center lg:flex">
          <nav className="flex items-center gap-2 border border-white/10 bg-black/20 px-3 py-2 text-[10px] font-black uppercase tracking-[0.22em] text-white/58 shadow-[0_24px_80px_rgba(0,0,0,0.24)] backdrop-blur-xl">
            {shinobiNav.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="px-4 py-3 transition duration-300 hover:bg-white hover:text-black"
              >
                {item.label}
              </Link>
            ))}
        ...
        <div className="hidden justify-end lg:flex">
          <Link
            ...
            target="_blank"
            rel="noreferrer"
            className="group inline-flex items-center gap-3 border border-white/14 bg-white px-5 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-black shadow-[0_24px_80px_rgba(0,0,0,0.24)] transition duration-300 hover:-translate-y-0.5 hover:border-[#F4261C] hover:bg-[#F4261C] hover:text-white"
          >
            Pedir agora
            <ArrowUpRight
              size={14}
        ...
        >
          ...
          <nav className="relative grid flex-1 content-center px-6">
            {shinobiNav.map((item, index) => (
              ...
              >
                <span className="text-xs font-black uppercase tracking-[0.2em] text-[#F4261C]">
                  0{index + 1}
                </span>
                <span className="text-4xl font-black uppercase tracking-[-0.05em] transition group-hover:text-[#F4261C]">
                  {item.label}
                </span>
                <ArrowUpRight
                  size={22}
          ...
          <div
            className={`relative grid gap-5 p-6 transition duration-500 ${
              isOpen ? "translate-y-0 opacity-100 delay-[340ms]" : "translate-y-4 opacity-0"
            }`}
          >
            ...
            <Link
              ...
              rel="noreferrer"
              onClick={closeMenu}
              className="group inline-flex items-center justify-center gap-3 bg-white px-6 py-5 text-[11px] font-black uppercase tracking-[0.22em] text-black transition hover:bg-[#F4261C] hover:text-white"
            >
              Pedir no WhatsApp
              <ArrowUpRight
                size={15}
