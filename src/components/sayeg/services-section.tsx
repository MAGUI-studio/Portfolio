...
import { SayegImageReveal, SayegReveal } from "./motion";
...
export function SayegServicesSection() {
  return (
    <section id="especialidades" className="relative bg-[#fafafa] px-5 pb-28 pt-20 md:px-10 md:pb-36 md:pt-28 lg:px-14 lg:pb-44 lg:pt-36">
      <div>
        <SayegReveal className="grid gap-7 lg:grid-cols-[0.62fr_0.38fr] lg:items-end">
          <div>
        ...
        <div className="mt-12 grid gap-10 md:grid-cols-3 md:gap-5 lg:gap-7">
          {sayegServices.map((service, index) => (
            <SayegReveal key={service.number} delay={index * 0.08} className={index === 1 ? "md:pt-14" : index === 2 ? "md:pt-7" : ""}>
            <article className="group">
              <SayegImageReveal delay={index * 0.08} className="relative aspect-[1.16] overflow-hidden bg-[#dedede]">
              ...
              <div className="relative z-10 -mt-7 ml-4 bg-[#fafafa] p-5 md:ml-3 md:p-4 lg:ml-5 lg:p-6">
                ...
                <Link href={sayegWhatsapp} target="_blank" rel="noreferrer" className="group/link mt-5 inline-flex items-center gap-2 border-b border-[#673d35]/65 pb-1 text-[10px] font-bold uppercase tracking-[0.2em] text-[#673d35] transition-colors hover:border-[#262323] hover:text-[#262323] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#8d594d] focus-visible:ring-offset-4">Conhecer a técnica <ArrowUpRight size={14} className="transition-transform duration-300 group-hover/link:translate-x-0.5 group-hover/link:-translate-y-0.5" /></Link>
              </div>
            </article>
            </SayegReveal>
          ))}
